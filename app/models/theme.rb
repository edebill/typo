class Theme
  ...
  def layout(action=:default)
    if action.to_s == 'view_page'
      if File.exists? "#{Rails.root}/themes/#{name}/layouts/pages.html.erb"
        return "#{Rails.root}/themes/#{name}/layouts/pages.html.erb"
      end
    end
    "#{Rails.root}/themes/#{name}/layouts/default.html.erb"
  end
