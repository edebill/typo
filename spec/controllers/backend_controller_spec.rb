...
describe BackendController do
  ...
  it "test_meta_weblog_new_media_object" do
    media_object = MetaWeblogStructs::MediaObject.new(
      ...
      "type" => "image/gif",
      "bits" => Base64.encode64(File.open(File.expand_path(Rails.root) + "/public/images/powered.gif", "rb") { |f| f.read })
    )
    ...
    assert File.unlink(File.expand_path(Rails.root) + "/public/files/#{media_object['name']}")
  end
