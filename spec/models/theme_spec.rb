...
describe 'Given a new test theme' do
  ...
  it 'layout path should be "#{Rails.root}/themes/test/layouts/default.html.erb"'  do
    @theme.layout('index').should == "#{Rails.root}/themes/test/layouts/default.html.erb"
  end  
end
...
describe 'Given the default theme' do
  ...
  it 'theme description should be correct' do
    @theme.description.should ==
      File.open(Rails.root + '/themes/typographic/about.markdown') {|f| f.read}
  end

  it 'theme_from_path should find the correct theme' do
    Theme.theme_from_path(Rails.root + 'themes/typographic').name.should == 'typographic'
    Theme.theme_from_path(Rails.root + 'themes/scribbish').name.should == 'scribbish'
  end

  it '#search_theme_path finds the right things' do
    Theme.should_receive(:themes_root).and_return(Rails.root + '/test/mocks/themes')
