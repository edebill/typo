...
require 'fileutils'
FM_ID = 'filemanager'
FM_ROOT = File.join(Rails.root, 'vendor', 'plugins', FM_ID)
...
FileUtils.cp_r(fm_rails_path, File.join(Rails.root, 'vendor', 'plugins'))
FileUtils.cp_r(FM_PATH_PUBLIC, Rails.root)
FileUtils.rm_rf(FM_PATH_PUBLIC)
end
=end

FM_CONFIG_FILE = File.join(Rails.root, 'config', 'filemanager.yml')
